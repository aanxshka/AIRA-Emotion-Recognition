...
DETECTOR_BACKEND = "retinaface"   # <- changed from DeepFace default to RetinaFace
...
# Same logic as your DeepFace version, but RetinaFace is used for detection
# ─────────────────────────────────────────────
        ...
        if frame_idx in sample_indices:
            try:
                result = DeepFace.analyze(
                    frame,
                    actions=["emotion"],
                    detector_backend=DETECTOR_BACKEND,   # <- key change
            ...
            cv2.imshow(f"Processing video… [{DETECTOR_BACKEND}]", frame)
            cv2.waitKey(1)
...
def make_run_dir() -> str:
    ...
    n = 1
    while True:
        run_dir = os.path.join("results", f"deepface_retinaface{n}")
...
def main():
    ...
    print(f"\n=== DeepFace Video Evaluator [{DETECTOR_BACKEND}] ===")
    ...
    while True:
        ...
        # Step 3 — run DeepFace with RetinaFace backend
    ...
    if rows:
        ...
        print("   summary.csv  — overall accuracy")
