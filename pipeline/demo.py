"""
AIRA Emotion Detection Demo

Real-time multimodal emotion detection with per-user calibration.
...
Outputs CSV logs to dashboard/data/emotion_data.csv for the
...
# MLP model path
MLP_MODEL_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)),
...
class DeepFaceAudioFusionApp:
    ...
    def __init__(self, camera_index: int = 0):
        ...
        # Signal quality tracking (rolling window for CSV logging)
        self._signal_window = 50  # track last N frames/chunks
        self._video_detections: List[bool] = []   # True=face detected, False=not
        self._audio_detections: List[bool] = []   # True=speech detected, False=skipped
        ...
        # GUI
        self.root = tk.Tk()
        self.root.title("AIRA Emotion Detection")
    ...
    def save_profile(self):
        """Calibration is in-memory only for this demo."""
        messagebox.showinfo("Info", "Calibration is stored in memory for this session.")

    def load_profile(self):
        """Calibration is in-memory only for this demo."""
        messagebox.showinfo("Info", "Please use Calibrate to create a new profile.")
    ...
    def _log_to_csv(self, fusion_result, face_for_fusion, audio_result,
                    face_stale, audio_stale):
        """Append one row to the CSV for dashboard consumption."""
        ...
        row = {
            ...
            'primary_emotion': fusion_result.emotion,
            'confidence': round(fusion_result.confidence * 100, 1),
            'happy_score': round(fused_probs.get('Happy', 0) * 100, 1),
            'sad_score': round(fused_probs.get('Sad', 0) * 100, 1),
            'fear_score': round(fused_probs.get('Fear', 0) * 100, 1),
            'angry_score': round(fused_probs.get('Angry', 0) * 100, 1),
            'disgust_score': round(fused_probs.get('Disgust', 0) * 100, 1),
            'neutral_score': round(fused_probs.get('Neutral', 0) * 100, 1),
            'surprise_score': round(fused_probs.get('Surprise', 0) * 100, 1),
            'video_signal_quality': round(
                sum(self._video_detections) / len(self._video_detections) * 100, 1
            ) if self._video_detections else 0.0,
            'audio_signal_quality': round(
                sum(self._audio_detections) / len(self._audio_detections) * 100, 1
    ...
    def update_fusion_display(self):
        ...
        if face_cal and not face_stale:
            ...
            tag = {'calibration': '[CAL]', 'raw_model': '[RAW]',
                   ...
                   '': '',  # No calibration active — show no tag
                   }.get(source, f'[{source}]')
        ...
        self._log_to_csv(fusion_result, face_for_fusion, audio_result,
                         face_stale, audio_stale)
    ...
    def face_loop(self):
        while self.running:
            try:
                ...
                # Track video signal quality
                self._video_detections.append(face_bgr is not None)
                if len(self._video_detections) > self._signal_window:
                    self._video_detections = self._video_detections[-self._signal_window:]
    ...
    def audio_loop(self):
        ...
        while self.running:
            try:
                ...
                # Track audio signal quality
                self._audio_detections.append(has_speech)
                if len(self._audio_detections) > self._signal_window:
                    self._audio_detections = self._audio_detections[-self._signal_window:]
